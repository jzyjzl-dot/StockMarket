  ...
  <div class="account-group-edit">
      ...
      <div class="content">
        ...
        <div class="search-bar" style="margin-bottom: 20px;">
          <el-row :gutter="20">
            ...
            <el-col :span="4">
              <el-button type="info" @click="showGroupStats" style="width: 100%">
        ...
        >
          ...
          <el-table-column label="操作" min-width="100" fixed="right">
            <template #default="scope">
              <el-button size="small" @click="viewGroupAccounts(scope.row)">查看账户</el-button>
    ...
    <el-dialog v-model="showEditDialog" title="编辑账户组" width="600px">
      <el-form :model="editForm" label-width="100px">
        <el-form-item label="组ID">
          <el-input-number v-model="editForm.groupId" :min="1" :precision="0" style="width: 100%"></el-input-number>
        </el-form-item>
    ...
    <el-dialog v-model="showAddDialog" title="添加账户组" width="600px">
      <el-form :model="addForm" label-width="100px">
        <el-form-item label="组ID">
          <el-input-number v-model="addForm.groupId" :min="1" :precision="0" style="width: 100%"></el-input-number>
        </el-form-item>
        <el-form-item label="描述">
    ...
    <!-- 查看账户对话框 -->
    <el-dialog v-model="showAccountsDialog" :title="`查看账户 - 组 ${selectedGroup?.groupId}`" width="80%">
      ...
      <el-table :data="groupAccounts" style="width: 100%" max-height="400px" stripe>
        ...
        <el-table-column label="操作" width="120">
          <template #default="scope">
            <el-button size="small" type="danger" @click="removeAccountFromGroup(scope.row)">移除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
    ...
    <!-- 添加账户到组对话框 -->
    <el-dialog v-model="showAddAccountDialog" :title="`添加账户到组 ${selectedGroup?.groupId}`" width="600px">
      <div style="margin-bottom: 20px;">
        <el-input
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="addAccountsToGroup" :disabled="selectedAccounts.length === 0">
    ...
    <el-dialog v-model="showStatsDialog" title="账户组统计" width="600px">
      <div class="stats-content">
        ...
        <div class="group-stats">
          <div v-for="group in accountGroups" :key="group.id" class="group-stat-item">
            <div class="group-info">
              <span class="group-name">组 {{ group.groupId }}</span>
              <span class="group-count">{{ group.accountCount }} 个账户</span>
            </div>
            <div class="group-balance">${{ group.totalBalance.toLocaleString() }}</div>
          </div>
        </div>
      </div>
      <template #footer>
...
import { Search, RefreshRight, Plus, DataAnalysis, Money } from '@element-plus/icons-vue'
...
const availableAccounts = computed(() => {
  ...
  return allAccounts.value.filter(account => account.group !== selectedGroup.value.groupId.toString())
})
...
const totalAssets = computed(() => {
  return processedGroups.value.reduce((sum, group) => sum + group.totalBalance, 0)
})
...
const viewGroupAccounts = (group) => {
  ...
  groupAccounts.value = allAccounts.value.filter(account => account.group === group.groupId.toString())
...
const removeAccountFromGroup = async (account) => {
  try {
    await ElMessageBox.confirm(`确定要将账户 "${account.accountName}" 移除出组 ${selectedGroup.value.groupId} 吗？`, '提示', {
    ...
    // 重新加载当前组的账户列表
    groupAccounts.value = allAccounts.value.filter(acc => acc.group === selectedGroup.value.groupId.toString())
  } catch (error) {
    if (error !== 'cancel') {
...
const addAccountsToGroup = async () => {
  try {
    ...
    ElMessage.success(`已添加 ${selectedAccounts.value.length} 个账户到组 ${selectedGroup.value.groupId}`)
...
const saveGroup = async () => {
  try {
    // 这里可以添加保存组信息的逻辑
    // 目前只是更新本地数据
    const index = accountGroups.value.findIndex(g => g.id === editForm.value.id)
    if (index !== -1) {
      accountGroups.value[index] = { ...editForm.value, lastUpdated: new Date().toISOString() }
    }
...
const deleteGroup = async (group) => {
  try {
    await ElMessageBox.confirm(`确定要删除组 ${group.groupId} 吗？此操作将影响 ${group.accountCount} 个账户。`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
    ...
    // 将该组的所有账户移到默认组
    const accountsToUpdate = allAccounts.value.filter(account => account.group === group.groupId.toString())
